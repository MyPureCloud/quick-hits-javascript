// >> START external-contacts Guide on creating External Contacts from a CSV file
const platformClient = require('purecloud-platform-client-v2');
const CSV = require('csv-string');
const fs = require('fs');
const client = platformClient.ApiClient.instance;

// >> START external-contacts-step-1
...
client.loginClientCredentialsGrant(CLIENT_ID, CLIENT_SECRET)
	.then(() => {
// >> END external-contacts-step-1
		console.log('Authenticated with Genesys Cloud');
		// >> START external-contacts-step-2
		const organization = {
	...
	.then((organization) => {
		...
		// >> END external-contacts-step-2
		// >> START external-contacts-step-3
		const promises = [];
	...
	.then(() => {
		...
		// >> END external-contacts-step-3
	})
	.catch((err) => console.log(err));
// >> END external-contacts
