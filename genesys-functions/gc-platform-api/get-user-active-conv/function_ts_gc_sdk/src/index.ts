// >> START get-user-active-conv-gc-sdk-typescript Example of simple NodeJS function (typescript) for Genesys Cloud Function to perform Genesys Cloud Platform API requests

// Import needed built in and external libraries.
import { Handler } from 'aws-lambda';
import { verifyGCToken, requestGCToken, findGCUser, findActiveConversationsForGCUser } from './gc_sdk_utils.js';
import { default as platformClient } from 'purecloud-platform-client-v2';

// >> START get-user-active-conv-gc-sdk-typescript-step-1
var functionLocalCache_GCToken: string | null = null;
// >> END get-user-active-conv-gc-sdk-typescript-step-1

// >> START get-user-active-conv-gc-sdk-typescript-step-2
...
// >> END get-user-active-conv-gc-sdk-typescript-step-2
    ...
    try {
        ...
        // >> START get-user-active-conv-gc-sdk-typescript-step-3
        ...
        // >> END get-user-active-conv-gc-sdk-typescript-step-3
        ...
        // >> START get-user-active-conv-gc-sdk-typescript-step-4
        // Find user based on username, email or employeeId
        let searchUserResults: platformClient.Models.UsersSearchResponse  = await findGCUser(gcRegion, gcToken, inputType, inputId);
        let userId = null;
        if (searchUserResults && searchUserResults.total === 0) {
            ...
            if (searchUserResults && searchUserResults.results && (searchUserResults.results as any[]).length > 0) {
                ...
                if (firstUser.state === 'active') {
                    ...
                    if (nbActiveConversationsFromConversationSummary > 0) {
                        ...
                        let searchUserResults: platformClient.Models.AnalyticsConversationQueryResponse = await findActiveConversationsForGCUser(gcRegion, gcToken, userId, mediaType);
            ...
            // >> END get-user-active-conv-gc-sdk-typescript-step-4
        }
    } catch (error) {
        // >> START get-user-active-conv-gc-sdk-typescript-step-5
        ...
        // >> END get-user-active-conv-gc-sdk-typescript-step-5
    }
};
// >> END get-user-active-conv-gc-sdk-typescript
